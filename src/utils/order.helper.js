...
export const paymentMethod = order =>
  order.paymentType === "CASH"
    ? "Cash : " +
      (order.checkoutType === "DELIVERY" ? "Charge On Delivery!" : "Store")
    : "Paid by Card";

export const checkoutType = type =>
  type === "DELIVERY" ? "Delivery" : "Pick-up";

export const orderTime = order =>
  order.expectedTime === "NOW"
    ? "Now"
    : moment(order.scheduled).format("MM/DD/YYYY hh:mm");

const orderItemGroups = items => items.map(basketItem => basketItem.group.name);
const orderGroupItems = items => groupName =>
  items.filter(basketItem => basketItem.group.name === groupName);

const orderItemsAsArray = obj =>
  Object.keys(obj).map(key => {
    return { group: key, items: obj[key] };
  });

export const orderItems = order => {
  return orderItemsAsArray(
    orderItemGroups(order.userBasket).reduce((prev, groupName) => {
...
export const itemDetails = item => {
  const details =
    item.selectedName ||
    item.selections.map(selection => selection.selectedName).join(", ");
  return {
    type: item.type,
    description: details
  };
...
const setProductPropsForRender = properties => {
  ...
  const result = Object.keys(properties)
    ...
    .reduce((prev, current) => {
      prev[current] =
        typeof properties[current] === "object"
...
export const orderItemsByGroup = order => {
  const result = order.userBasket.reduce((prev, current) => {
